...
class OfflineControllerWithSmallRotation(BaseController):
    ...
    def __init__(
        ...
        self.detection_feature_file_name = detection_feature_file_name
        self.images_file_name = images_file_name
        self.optimal_action_file_name = optimal_action_file_name
        self.depth_file_name = depth_file_name
        self.grid = None
        self.graph = None
        self.metadata = None
        self.images = None
        self.detection_feature = None
    ...
    def reset(self, scene_name=None):
        ...
        if scene_name != self.scene_name:
            ...
            self.detection_feature = self.h5py.File(
                os.path.join(
                    self.offline_data_dir, self.scene_name, self.detection_feature_file_name
                ),
                'r',
            )
            ...
            if self.optimal_action_file_name is not None:
                with open(
                        os.path.join(
                            self.offline_data_dir, self.scene_name, self.optimal_action_file_name
                        ),
                        'r'
                ) as rf:
                    self.optimal_action = json.load(rf)
    ...
    def all_objects(self):
        if self.using_raw_metadata:
            ...
            return [key for key in self.metadata if len(self.metadata[key])!=0]

    def get_optimal_action(self, target_class):
        return self.optimal_action[str(self.state)][target_class]

    def get_depth(self):
        return self.depth[str(self.state)][()]
